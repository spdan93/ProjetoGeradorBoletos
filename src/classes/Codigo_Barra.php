...
function geraCodigoBarra($numeroOriginal)
{
	$fino = 1;
	$largo = 3;
	$altura = 50;

	if (strlen($numeroOriginal) >= 47) {
		$numero = preg_replace('/(\d{4})(\d{5})\d{1}(\d{10})\d{1}(\d{10})\d{1}(\d{15})/', '$1$5$2$3$4', str_replace([" ", ".", "-"], "", $numeroOriginal));
	} else if (strlen($numeroOriginal) == 44) {
		$numero = $numeroOriginal;
	} else {
		echo "Número de boleto inválido";
		exit();
	}
	...
	echo '<br>' . $numeroOriginal;
}

geraCodigoBarra($_POST['data']);